...
func NewServer(address string, devices []*uac1.Device, logger *log.Logger) *Server {
	...
	return &Server{Address: address, Devices: devices, Logger: logger, ready: make(chan struct{}), connections: make(chan struct{}, 32)}
}
...
func (s *Server) Serve(ctx context.Context) error {
	...
	if err != nil || host != "127.0.0.1" {
		return fmt.Errorf("USB/IP listener must use loopback")
	}
	...
	for {
		...
		go func() { defer func() { <-s.connections }(); s.handleConnection(ctx, conn) }()
	}
}
...
func (s *Server) handleConnection(ctx context.Context, conn net.Conn) {
	...
	case OpReqImport:
		if !kernelPeer(conn) || !s.imported.CompareAndSwap(false, true) {
			_ = writeOpHeader(conn, OpRepImport, 1)
			return
		}
		defer s.imported.Store(false)
		busRaw := make([]byte, 32)
...
func (s *Server) writeDeviceList(w io.Writer) error {
	...
	for _, d := range s.Devices {
		...
		interfaces := [][4]byte{
			{0x01, 0x01, 0x00, 0}, // AudioControl
			{0x01, 0x02, 0x00, 0}, // AudioStreaming capture
		}
...
func writeUSBDevice(w io.Writer, d *uac1.Device) error {
	var path [256]byte
	var busID [32]byte
	fixedString(path[:], fmt.Sprintf("/sys/devices/platform/micweave/%s", d.BusID))
	...
	fields := []any{
		...
		uint8(2), // capture-only: AudioControl + AudioStreaming
	}
...
func (c *connectionState) writeSubmit(req SubmitRequest, status int32, actual uint32, data []byte, packets []IsoPacket, errorCount uint32) error {
	...
	c.mu.Lock()
	if pending := c.pending[req.Basic.Sequence]; pending != nil {
		if pending.unlinked {
			c.mu.Unlock()
			return nil
		}
	...
	c.mu.Unlock()
	if conn, ok := c.writer.(net.Conn); ok {
...
func (c *connectionState) addPending(sequence uint32, cancel context.CancelFunc) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.pending[sequence] != nil || len(c.pending) >= maxPendingRequests {
		return false
	}
	c.pending[sequence] = &pendingRequest{cancel: cancel}
	return true
}
...
func (c *connectionState) cancelPending(sequence uint32) bool {
	...
	if pending == nil || pending.completing || pending.unlinked {
		return false
	}
...
func readSubmit(r io.Reader, basic BasicHeader) (parsedSubmit, error) {
	if basic.Direction > DirectionIn || basic.Endpoint > 1 || (basic.Endpoint == 1 && basic.Direction != DirectionIn) {
		return parsedSubmit{}, fmt.Errorf("invalid endpoint or direction")
	}
	...
	if basic.Endpoint == 0 && len(packets) > 0 {
		return parsedSubmit{}, fmt.Errorf("isochronous control request")
	}
	var total uint64
	for _, p := range packets {
		total += uint64(p.Length)
		if p.Length > 192 || uint64(p.Offset)+uint64(p.Length) > uint64(req.TransferBufferLength) || total > uint64(req.TransferBufferLength) {
			return parsedSubmit{}, fmt.Errorf("invalid isochronous packet bounds")
		}
	}
...
func (s *Server) handleURBs(ctx context.Context, r *bufio.Reader, w io.Writer, device *uac1.Device) {
	...
	for {
		...
		if basic.DeviceID != uint32(1<<16|device.Number) {
			return
		}
		...
		case CmdSubmit:
			...
			if !state.addPending(basic.Sequence, requestCancel) {
				requestCancel()
				return
			}
			state.wg.Add(1)
...
func (s *Server) processSubmit(ctx context.Context, state *connectionState, device *uac1.Device, submit parsedSubmit) error {
	...
	switch {
	case req.Basic.Endpoint == 1 && req.Basic.Direction == DirectionIn:
		if req.IsIsochronous() {
